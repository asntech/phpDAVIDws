...
if($response->getReturn() == "true")
{
	...
	$response = $service->getAllAnnotationCategoryNames(new getAllAnnotationCategoryNamesResponse($service));
	echo $response->getReturn();
	 ...
	 $request = new addList($inputIds, $idType, $listName, $listType);
	 $response = $service->addList($request);
	 echo $response->getReturn();

	 //getConversionTypes
	 $response = $service->getConversionTypes(new getConversionTypesResponse($service));
	 echo $response->getReturn();
